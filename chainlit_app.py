...
def load_documents_from_azure():
    ...
    try:
        ...
        current_pdfs = {b.name for b in container_client.list_blobs() if b.name.endswith(".pdf")}
        cached_pdfs  = {doc["filename"] for doc in cached_docs}
        if current_pdfs == cached_pdfs:
            return cached_docs
    except Exception:
        cached_docs = []

    all_text         = list(cached_docs)
    cached_filenames = {doc["filename"] for doc in all_text}
    ...
    for blob in container_client.list_blobs():
        if blob.name.endswith((".xlsx", ".xls")) and blob.name not in cached_filenames:
            blob_client = container_client.get_blob_client(blob.name)
    ...
    for blob in container_client.list_blobs():
        if blob.name.endswith((".docx", ".doc")) and blob.name not in cached_filenames:
            blob_client = container_client.get_blob_client(blob.name)
            file_bytes  = blob_client.download_blob().readall()
...
def detect_color_content(image):
    ...
    saturation = hsv[:, :, 1]
    return np.sum(saturation > 50) / saturation.size
            ...
            response = client.chat.completions.create(
                ...
                messages=[{
                    ...
                    "content": [
                        ...
                        {"type": "text", "text": (
                            "You are reading a construction document. Extract ALL text including upside down or "
                            "rotated text. Mark circled items as CIRCLED: [item], highlighted text as "
                            "HIGHLIGHTED: [item], handwritten notes as ANNOTATION: [note], red markups as "
                            "REVISION: [content], stamps as STAMP: [content]. Ignore coffee stains and paper "
                            "damage. Note color coded elements."
                        )}
                    ]
                }]
            )
...
async def ask_question_stream_async(question, documents):
    doc_keywords = [
        "what documents", "what do you have", "what's in the database",
        "whats in the database", "what files", "what data do you have",
        "what can you access", "what information do you have"
    ]
    if any(phrase in question.lower() for phrase in doc_keywords):
        yield ("Fieldbook has access to P&J project documents, specifications, subcontracts, "
               "plumbing and HVAC drawings, submittal logs, RFIs, email correspondence, and live "
               "Procore project data. Ask me anything specific and I'll find it.")
        return
    ...
    if pinecone_context:
        ...
        procore_keywords = ["rfi", "submittal", "procore", "procure", "prorcore",
                            "approved", "rejected", "pending", "project"]
            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "Answer questions based on the provided construction documents and any "
                        "Procore project data included in the context. "
    ...
    try:
        ...
        email_keywords = ["email", "wrote", "sent", "said", "submittal", "approved",
                          "engineer", "correspondence", "message", "confirm", "rfi",
                          "vendor", "contractor"]
        question_words = question.lower().split()
        email_trigger  = any(
            any(fuzz.ratio(qword, kw) > 80 for kw in email_keywords)
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "Answer questions based on the provided construction documents and any "
                    "Procore project data included in the context. "
                    "Always cite the document name, section title, and page number in your answer. "
                    "If the answer is not in the documents, say so clearly.\n\n"
...
def _preload():
    ...
    try:
        _preloaded_docs = load_documents_from_azure()
    except Exception as e:
...
@cl.on_chat_start
async def start():
    cl.user_session.set("documents", _preloaded_docs)
...
async def main(message: cl.Message):
    ...
    try:
        async for chunk in ask_question_stream_async(message.content, documents):
